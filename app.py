    ...
    if st.button("✨ Generate Full Suite"):
        if st.session_state.pitch_count >= 3:
            ...
            with st.spinner('Scraping & Researching...'):
                ...
                prompt = f"Company: {company_name} | Info: {raw_data} | Sell: {product} | Tone: {tone}. Generate Cold Email, LinkedIn DM, WhatsApp, Elevator Pitch, and 3 Pain Points."
                ...
                if full_pitch:
                    ...
                    st.error("⚠️ All Engine slots are busy. Google Free Tier is throttled. Please wait 60 seconds.")
...
with col_out:
    if 'current_pitch' in st.session_state:
        ...
        st.download_button("📄 Download PDF Report", pdf_bytes, f"{st.session_state.current_company}_pitch.pdf", "application/pdf")
    else:
